...
// Range Configuration
const START_INDEX = Number(process.argv[2]) || 6000; // 0-based index for 6001th item
const END_INDEX = Number(process.argv[3]) || 10000;   // up to 10000th item
...
const OUTPUT_EXCEL = path.resolve(
  __dirname,
  `../google-place-ids-${START_INDEX + 1}-to-${END_INDEX}-scraped.xlsx`
);
const STATE_JSON = path.resolve(
  __dirname,
  `../google-place-ids-${START_INDEX + 1}-to-${END_INDEX}-state.json`
);
...
async function startRangeScrape() {
  console.log(`====================================================`);
  console.log(`🛡️ RANGE BATCH SCRAPER INITIALIZED`);
  console.log(`Range: Place IDs #${START_INDEX + 1} to #${END_INDEX} (${END_INDEX - START_INDEX} items)`);
  console.log(`All Enterprise & Pro Fields Included`);
  console.log(`Est. Cost: ~$${(((END_INDEX - START_INDEX) / 1000) * 6).toFixed(2)} USD (100% Covered by Free Credit)`);
  ...
  console.log(`\n====================================================`);
  console.log(`✅ RANGE SCRAPE COMPLETED SUCCESSFULLY`);
...
startRangeScrape().catch((err) => {
  console.error("Range scrape failed:", err);
  process.exit(1);
});
