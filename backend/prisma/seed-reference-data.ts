/**
 * Cities, localities and amenities reference data — extracted out of seed.ts so it can run
 * independently of that script's generic/placeholder category list. For the real production
 * taxonomy, use src/importer/bootstrap-categories.ts instead of seed.ts's category seeding.
 *
 * Safe to re-run: upserts by slug, same non-destructive pattern as seed.ts.
